...
# To do this, compare whether df_urb_pop['CountryCode'] is equal to 'CEB' within the square brackets in df_urb_pop[____].
...
# Hint
# Pass the file in 'ind_pop_data.csv' and a value of 1000 to the keyword argument chunksize to pd.read_csv().
# Use next() with urb_pop_reader as the argument to get the first DataFrame chunk.
# To select the correct rows, the condition inside df_urb_pop[] should be df_urb_pop['CountryCode'] == 'CEB'.
# Compare df_urb_pop['CountryCode'] if it is equal to 'CEB' within the square brackets in df_urb_pop[____].
# Call zip(), passing to it df_pop_ceb['Total Population'] and df_pop_ceb['Urban population (% of total)'] as arguments, in that order.